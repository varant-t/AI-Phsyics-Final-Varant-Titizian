...
public class KinematicInput : MonoBehaviour
{
    ...
    private void OnTriggerStay(Collider other)
    {
        if (other.gameObject.tag == "Ice")
        {
            ...
            //Causes the player to slide when it jumps and moves on the yellow platforms based off of continues mass and acceleration.
        ...
        if(other.gameObject.tag == "AI_2")
        {
            

                  // If the time is below the threshold, add the delta time
            if (timeColliding < explodingThreshold)
            {
                timeColliding += Time.deltaTime;
            }
            else
            {
                // Time is over theshold, player takes damage
                aiAnim2.ResetTrigger("Chasing");
                aiAnim2.SetTrigger("Explode");
                TakeDamage(25);
                // Reset timer
                timeColliding = 0f;
            }
    ...
    private void OnTriggerExit(Collider other)
    {
            ...
            if(other.gameObject.tag == "AI_2")
        {
            aiAnim2.ResetTrigger("Explode");
            aiAnim2.SetTrigger("Chasing");
            timeColliding = 0f;
        }
    }
